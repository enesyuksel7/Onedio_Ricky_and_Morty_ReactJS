    ...
    <>
      ...
      {!characters ? (
        ...
        <div className="bg-[#141227]">
			...
			<div className="grid grid-cols-1 gap-20 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
			{characters.map((character) => (
				<Cards key={character.id} {...character} />
			  ))}
			</div>
			<div className="py-16 font-bold flex justify-center">
				<button onClick={()=>showMore()} class="bg-[#23233F] hover:bg-[#B2B4D7] text-[#B2B4D7] font-bold py-2 px-32 rounded inline-flex items-center">
		...
		<footer className="p-6 text-[30px] text-center text-[#B2B4D7] bg-[#23233F]">
				<div class="text-sm text-blueGray-500 font-semibold py-1">
				<span id="get-current-year">© 2022</span> Onedio. All rights reserved
				</div>
		</footer>
		</div>
      )}
